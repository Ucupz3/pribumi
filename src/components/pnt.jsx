...
export default function NusaTenggara({ onMarkerClick, userXP }) {
    ...
    const getData = async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          ...
          islands!inner(slug)
        `)
        .eq("islands.slug", "bali-nusa-tenggara");

      if (error) {
        console.error("Gagal ambil marker Nusa Tenggara:", error.message);
      } else {
        setMarkers(data || []);
      }
    };
    ...
    <div className="absolute top-[90%] left-[66%] -translate-x-1/2 -translate-y-1/2 z-10">
      <div className="relative">
        <img
          ...
          className="w-[74%] min-w-[140px] h-auto pointer-events-none select-none"
        />

        {markers?.map((m) => {
          // ✅ Gunakan userXP dari props untuk logika buka gembok
