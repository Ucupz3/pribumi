...
export default function Sulawesi({ onMarkerClick, userXP = 0 }) {
    ...
    const fetchMarkers = async () => {
      const { data, error } = await supabase
        ...
        .select(`
          ...
          islands!inner(slug)
        `)
        .eq("islands.slug", "sulawesimlk"); // 🔥 penting
  ...
  return (
    <div className="absolute top-[44%] left-[62%] -translate-x-1/2 -translate-y-1/2 z-10">
      <div className="relative">
        <img src="/images/Pulau/Malaka.png" alt="Pulau Sulawesi" className="w-[74%] min-w-[160px] h-auto" />
        ...
        {markers.map((m) => {
          ...
          return (
            <div
              ...
              style={{ top: m.pos_top, left: m.pos_left, transform: "translate(-50%,-50%)" }}
            >
              {unlocked ? (
                <div onClick={(e) => handleClick(e, m, true)} className="cursor-pointer z-50">
                ...
                <div className="cursor-not-allowed">
                  <LockIcon size={26} />
                  <span className="absolute -top-7 left-1/2 -translate-x-1/2 bg-gray-800 text-yellow-400 text-[10px] px-2 py-0.5 rounded opacity-0 group-hover:opacity-100">
                    🔒 {m.xp_required} XP
                  </span>
                </div>
              )}
