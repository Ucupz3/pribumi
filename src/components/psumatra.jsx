...
// ✅ Menerima userXP dari props agar irit request
export default function Sumatra({ onMarkerClick, userXP }) {
  const [markers, setMarkers] = useState([]);

  useEffect(() => {
    const getData = async () => {
      // ✅ Hanya fetch data marker Sumatra dari Supabase
      ...
      if (error) {
        console.error("Gagal ambil marker Sumatra:", error.message);
      } else {
        setMarkers(data || []);
      }
    };
    ...
    <div className="absolute top-[56%] left-[34%] -translate-x-1/2 -translate-y-1/2 z-10">
      <div className="relative">
        <img
          ...
          alt="Pulau Sumatra"
          className="w-[38%] min-w-[150px] h-auto pointer-events-none select-none"
        />

        {markers.map((m) => {
          // ✅ Gunakan userXP dari props untuk validasi gembok
