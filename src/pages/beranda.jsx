...
async function getBerandaData() {
  ...
  return {
    ...
    quizHarian: {
      ...
      pilihan: ["Soekarno & Hatta", "Sudirman & Nasution", "Diponegoro & Kartini", "Gajah Mada & Hayam Wuruk"],
...
export default function Beranda() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center font-lora" style={bgPaper}>
  ...
  const handleClaim = (taskId) => {
    ...
    setTimeout(() => {
      setTasks(prev => prev.map(t =>
        t.id === taskId ? { ...t, sudahClaim: true } : t
      ));
      setClaimAnim(null);
    }, 600);
  };
...
return (
    <div className="h-screen lg:overflow-hidden overflow-auto flex flex-col font-lora" style={bgPaper}>
      ...
      <div className="flex-1 px-6 md:px-12 py-6 lg:overflow-hidden">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full">
          ...
          {/* ===== KIRI — Card Profil ===== */}
          <div className="border-2 border-[#c9b896] rounded-2xl overflow-hidden shadow-md flex flex-col" style={bgPaper}>
            ...
            <div className="px-6 py-4 space-y-4 flex-1">
              ...
              <div>
                <div className="flex justify-between mb-2">
                  <span className="text-[#5c4033] text-sm font-bold">XP Total</span>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-[#e8dcc0]/50 border border-[#c9b896] rounded-xl px-3 py-3 text-center">
                  <p className="text-[#BD9B2C] font-black text-xl">+{user.xpHariIni}</p>
                  <p className="text-[#a08060] text-[10px] mt-0.5">XP Hari Ini</p>
                </div>
                <div className="bg-[#e8dcc0]/50 border border-[#c9b896] rounded-xl px-3 py-3 text-center">
                  <p className="text-[#5c4033] font-black text-xl">{user.totalQuestSelesai}</p>
                  <p className="text-[#a08060] text-[10px] mt-0.5">Quest Selesai</p>
                </div>
                <div className="bg-[#e8dcc0]/50 border border-[#c9b896] rounded-xl px-3 py-3 text-center">
                  <p className="text-[#5c4033] font-black text-xl">#{user.rank}</p>
          ...
          <div className="flex flex-col gap-4 overflow-hidden">
            ...
            {/* Quiz Harian */}
            <div className="border-2 border-[#c9b896] rounded-2xl overflow-hidden shadow-md relative flex-shrink-0" style={bgPaper}>

              <div className="px-5 py-3 border-b-2 border-[#c9b896] flex items-center justify-between relative z-10">
                <div>
                  <p className="text-[#5c4033] font-black text-base">Quiz Harian ⚡</p>
                  <p className="text-[#a08060] text-xs">+{quizHarian.xpReward} XP · Ganti tiap hari</p>
                </div>
                {quizSudahJawab && (
              ...
              <div className="px-5 py-3 space-y-3 relative">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {quizHarian.pilihan.map((pil, i) => {
                    ...
                    if (quizSudahJawab) {
                      if (isBenar) { borderStyle = "border-[#4a7c59] bg-[#4a7c59]/10 text-[#4a7c59]"; hurufBg = "bg-[#4a7c59] text-white"; }
                      else if (isSelected) { borderStyle = "border-[#8b3a3a] bg-[#8b3a3a]/10 text-[#8b3a3a]"; hurufBg = "bg-[#8b3a3a] text-white"; }
                      else { borderStyle = "border-[#c9b896] text-[#a08060]"; }
                    }
                    return (
                      <button
                        key={pil}
                      ...
                      >
                        <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black flex-shrink-0 ${hurufBg}`}>
                          {huruf}
                        </span>
                ...
                {quizSudahJawab && (
                  <div
                    ...
                    style={{ ...bgPaper, backgroundColor: "rgba(255,255,255,0.88)", backdropFilter: "blur(4px)" }}
                  >
                    <span className="text-3xl">🎉</span>
                    <p className="text-[#5c4033] font-black text-sm">Quiz Selesai!</p>
                    <p className="text-[#a08060] text-xs text-center px-4">Kembali lagi besok!</p>
                    <p className={`text-xs font-bold mt-0.5 ${quizJawaban === quizHarian.jawaban ? "text-[#4a7c59]" : "text-[#8b3a3a]"}`}>
                      {quizJawaban === quizHarian.jawaban ? `✅ Benar! +${quizHarian.xpReward} XP` : `❌ Jawaban salah`}
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="border-2 border-[#c9b896] rounded-2xl overflow-hidden shadow-md flex flex-col flex-1 min-h-0" style={bgPaper}>
              <div className="px-5 py-3 border-b-2 border-[#c9b896] flex items-center justify-between flex-shrink-0">
                <div>
                  <p className="text-[#5c4033] font-black text-base">Task Mingguan 🎯</p>
                  <p className="text-[#a08060] text-xs">Ganti tiap 7 hari · Klaim XP-mu!</p>
                </div>
              ...
              ">
                {tasks.map((task) => {
                  const persen = Math.min((task.progress / task.target) * 100, 100);
                  ...
                  return (
                    <div
                      ...
                      className={`border-2 rounded-2xl px-4 py-3 transition-all duration-300 ${
                        isClaimed ? "border-[#4a7c59]/40 opacity-60"
                        : isFull ? "border-[#BD9B2C] shadow-md shadow-[#BD9B2C]/20"
                        : "border-[#c9b896]"
                      }`}
                      style={bgPaper}
                    >
                      <div className="flex items-center gap-3">
                        <span className={`text-2xl flex-shrink-0 ${isClaimed ? "grayscale" : ""}`}>{task.icon}</span>

                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2 mb-1">
                            <p className={`font-black text-sm truncate ${isClaimed ? "text-[#a08060] line-through" : "text-[#5c4033]"}`}>
                              {task.judul}
                            </p>
                            <span className={`text-[10px] font-black px-2 py-0.5 rounded-full flex-shrink-0 ${
                              isClaimed ? "bg-[#4a7c59]/20 text-[#4a7c59]" : "bg-[#BD9B2C]/20 text-[#81691A]"
                            }`}>
                              +{task.xpReward} XP
                            </span>
                          </div>
                          <p className="text-[#a08060] text-xs mb-2">{task.deskripsi}</p>
                          <div className="flex items-center gap-2">
                            <span className="text-[#a08060] text-[10px] flex-shrink-0">{task.progress}/{task.target}</span>
                            <div className="flex-1 h-2.5 bg-[#e8dcc0] rounded-full border border-[#c9b896] overflow-hidden">
                              <div
                                className={`h-full rounded-full transition-all duration-700 ${
                                  isClaimed ? "bg-[#4a7c59]"
                                  : isFull ? "bg-gradient-to-r from-[#BD9B2C] to-[#81691A]"
                                  : "bg-gradient-to-r from-[#c9b896] to-[#a08060]"
                                }`}
                                style={{ width: `${persen}%` }}
                              />
                            </div>
                            <span className="text-[#5c4033] text-[10px] font-bold flex-shrink-0">{Math.round(persen)}%</span>
                          </div>
                        </div>
